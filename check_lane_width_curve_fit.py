        ...
        if event.type in {'NONE', 'TIMER', 'TIMER_REPORT', 'EVT_TWEAK_L', 'WINDOW_DEACTIVATE'}:
            ...
            if hit:
                ...
                if lane['draw_cubic_curve_points'] == False: # 三次曲线点尚未显示，显示之。
                    cubic_curve_fitting_utils.draw_cubic_curve_fitting_result(road_id, section_id, lane_id)
                    lane['draw_cubic_curve_points'] = True
                else: # # 三次曲线点已经显示，隐藏之。
                    cubic_curve_fitting_utils.remove_cubic_curve_fitting_result(road_id, section_id, lane_id)
